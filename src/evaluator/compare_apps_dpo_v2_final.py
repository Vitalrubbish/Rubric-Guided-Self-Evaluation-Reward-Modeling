#!/usr/bin/env python3
"""Compare base and protected-routed DPO policy on the frozen APPS final523."""
...
def main() -> None:
    ...
    training_ids = {str(row.get("id")) for row in read_jsonl(args.training_preferences)}
    dev_ids = {str(row.get("id")) for row in read_jsonl(args.dpo_dev)}
    final_ids = set(base_by_id)
    training_overlap = final_ids & training_ids
    dev_overlap = final_ids & dev_ids
    if training_overlap or dev_overlap:
        raise AssertionError(
            f"final523 overlap: training={sorted(training_overlap)[:5]} dev={sorted(dev_overlap)[:5]}"
        )

    route_manifest = json.loads(args.route_manifest.read_text(encoding="utf-8"))
    if route_manifest.get("status") != "completed" or route_manifest.get("rows") != 523:
        raise AssertionError("protected route manifest is incomplete")
    if route_manifest.get("policy") != "verifier-free protected rubric; no execution result or reference answer is read":
        raise AssertionError("unexpected protected route policy")
    routed_rows = [row for row in candidate_rows if row.get("protected_rubric_route")]
    if len(routed_rows) != 523:
        raise AssertionError("candidate rows are missing protected rubric provenance")
    ...
    summary = {
        "status": "completed",
        "policy": "single frozen final523 evaluation; protected-rubric routed v5; no final tuning",
        "split_counts": dict(split_counts),
        "training_overlap_count": 0,
        "dpo_dev_overlap_count": 0,
        "decoding": base_decoding,
        "paired_verification": base_paired,
        "route_selection_counts": route_manifest.get("selection_counts"),
    ...
    summary["final_checks"] = {
        ...
        "negative_transitions_zero": combined["negative_transitions"] == 0,
        "training_overlap_zero": True,
    ...
    lines = [
        "# APPS DPO-v2 Protected-Rubric Final523",
        "",
        "| Split | Rows | Base pass@1 | Routed pass@1 | Net passes | + / - transitions |",
        "| --- | ---: | ---: | ---: | ---: | ---: |",
    ]
