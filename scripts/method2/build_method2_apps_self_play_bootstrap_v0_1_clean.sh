...
"$PYTHON" src/self_play/build_method2_bootstrap_data.py \
  ...
  --no-include-proxy \
  --sft-output "${SFT_OUTPUT:-data/sft/method2_apps_self_play_critic_repair_v0_1_clean.jsonl}" \
  --dpo-output "${DPO_OUTPUT:-data/preferences/method2_apps_self_play_critic_repair_pairs_v0_1_clean.jsonl}" \
  --summary-output "${SUMMARY_OUTPUT:-data/self_play/method2_apps_self_play_bootstrap_v0_1_clean_summary.json}" \
  --validation-percent "${VALIDATION_PERCENT:-10}"

echo "Method2 APPS self-play bootstrap v0.1 clean complete"
