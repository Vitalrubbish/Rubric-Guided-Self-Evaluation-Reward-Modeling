#!/usr/bin/env python3
"""Run a two-stage LLM critic -> specification -> revision loop."""
...
CODE_SYSTEM_PROMPT = (
    ...
    "Use the provided inferred specification to write a complete corrected solution. "
    "Return JSON only."
)
...
def short_tests(row: dict, max_chars: int = 1200) -> str:
...
def build_code_prompt(row: dict, spec_text: str) -> str:
    failed_code = clean_code(row.get("generated_code") or row.get("extracted_code") or "")
    return f"""Task prompt:
{row.get("prompt", "")[:1800]}
...
Write a complete corrected Python implementation.

Hard constraints:
- Preserve the required function name and call signature implied by the task/tests.
...
def parse_code_output(text: str) -> tuple[list[str], str]:
    obj = parse_json_object(text)
    if obj is not None:
...
def main() -> None:
    ...
    with args.output.open("w", encoding="utf-8") as f, torch.inference_mode():
        for index, row in enumerate(selected, start=1):
            ...
            spec_obj, normalized_spec_text = parse_spec(spec_text)

            code_text = generate_text(
                model,
                tokenizer,
                CODE_SYSTEM_PROMPT,
                build_code_prompt(row, normalized_spec_text),
                args.max_input_length,
                args.code_max_new_tokens,
                args.temperature,
                args.top_p,
            )
            findings, revised_code = parse_code_output(code_text)
            ...
            while compile_error_after_repair and repair_attempts_used < args.syntax_repair_attempts:
                ...
                repair_findings, repaired_code = parse_code_output(repair_text)
            ...
            record = {
                ...
                "sample_id": 0,
                "seed": args.seed,
                "prompt_mode": "two_stage_spec_code",
                "generation_backend": "transformers_two_stage_self_play_critic",
            ...
            print(
                f"generated two-stage revision {index}/{len(selected)}: {row['id']} "
                f"compile_error_after_repair={bool(compile_error_after_repair)}"
            )

    print(f"wrote {len(selected)} two-stage critic revisions to {args.output}")
