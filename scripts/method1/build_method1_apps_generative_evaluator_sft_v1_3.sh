...
OUTPUT=${OUTPUT:-data/sft/apps_simple_method1_generative_self_evaluator_v1_3_judge_focused.jsonl}
SPLIT_OUTPUT_DIR=${SPLIT_OUTPUT_DIR:-data/sft/apps_simple_method1_generative_self_evaluator_v1_3_judge_focused}
SUMMARY_OUTPUT=${SUMMARY_OUTPUT:-data/sft/apps_simple_method1_generative_self_evaluator_v1_3_judge_focused_summary.json}
...
"$PYTHON" -m src.evaluator.build_generative_evaluator_sft_data \
  ...
  --answer-first-judge \
  --train-task-types ${TRAIN_TASK_TYPES:-judge_single} \
  --heldout-task-types ${HELDOUT_TASK_TYPES:-judge_single} \
  --train-pass-repeat "${TRAIN_PASS_REPEAT:-1.5}" \
  --train-fail-repeat "${TRAIN_FAIL_REPEAT:-1.0}" \
  --train-primary-error-repeat "logic_other_or_unknown=${LOGIC_OTHER_REPEAT:-0.5}" \
  --train-primary-error-repeat "unclear_other_or_not_failure=${UNCLEAR_OTHER_REPEAT:-0.0}" \
  --sampling-seed "${SAMPLING_SEED:-42}"

echo "Generative evaluator SFT v1.3 judge-focused data complete: $OUTPUT"
