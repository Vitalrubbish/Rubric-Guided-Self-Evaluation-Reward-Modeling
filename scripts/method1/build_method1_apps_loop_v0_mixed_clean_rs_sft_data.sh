...
OUTPUT=${OUTPUT:-data/sft/apps_simple_method1_loop_v0_mixed_clean_rs_sft.jsonl}
SUMMARY_OUTPUT=${SUMMARY_OUTPUT:-data/sft/apps_simple_method1_loop_v0_mixed_clean_rs_sft_summary.json}
...
"$PYTHON" src/training/build_apps_loop_v0_mixed_rs_sft.py \
  ...
  --max-preservation-rows "${MAX_PRESERVATION_ROWS:-356}" \
  --validation-percent "${VALIDATION_PERCENT:-10}"

echo "Method1 loop-v0 mixed clean RS-SFT data complete:"
echo "  data: $OUTPUT"
